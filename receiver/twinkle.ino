...
void vizTwinkle(uint8_t speed) {
  ...
  for(uint8_t strand = 0; strand < NUM_STRANDS; strand++) {
    for(uint8_t pixel = 0; pixel < STRAND_LENGTH; pixel++) {
      CRGB color = getStrandModeColor(strand, pixel);
      leds[strand][pixel] = getTwinkleColor(PRNG16, clock32, color);
    }
  }
  for(uint8_t pixel = 0; pixel < LEDS_INNER; pixel++) {
    CRGB color = getInnerModeColor(pixel);
    dotsInner[pixel] = getTwinkleColor(PRNG16, clock32, color);
  }
  for(uint8_t pixel = 0; pixel < LEDS_OUTER; pixel++) {
    CRGB color = getOuterModeColor(pixel);
    dotsOuter[pixel] = getTwinkleColor(PRNG16, clock32, color);
  }
}
...
CRGB applyTwinkleBrightness(uint32_t ms, uint8_t salt, uint8_t speed, CRGB color) {
